...
def val(model, epoch, save_path, writer):
    ...
    with torch.no_grad():
        ...
        test_loader = test_dataset(image_root=opt.test_path + '/rgb/',
                                   polarization_root=opt.test_path + '/polarization/',
                            gt_root=opt.test_path + '/gt/',
                            testsize=opt.trainsize)

        for i in range(test_loader.size):
            image, polarization_image, gt, name = test_loader.load_data()
            gt = np.asarray(gt, np.float32)
            gt /= (gt.max() + 1e-8)
            image = image.cuda()
            polarization_image = polarization_image.cuda()
            res,res1 = model(image,polarization_image)
...
def train(train_loader, model, optimizer, epoch, test_path):
    ...
    for i, pack in enumerate(train_loader, start=1):
        for rate in size_rates:
            ...
            # print('this is i',i)
            # ---- data prepare ----
            images, polarization_images, gts = pack
            images = Variable(images).cuda()
            polarization_images = Variable(polarization_images).cuda()
            gts = Variable(gts).cuda()
            # ---- rescale ----
            trainsize = int(round(opt.trainsize * rate / 32) * 32)
            if rate != 1:
                images = F.upsample(images, size=(trainsize, trainsize), mode='bilinear', align_corners=True)
                polarization_images = F.upsample(polarization_images, size=(trainsize, trainsize), mode='bilinear', align_corners=True)
                gts = F.upsample(gts, size=(trainsize, trainsize), mode='bilinear', align_corners=True)
            # ---- forward ----
            # print('this is trainsize',trainsize)
            # P1, P2= model(images)
            P1,P2= model(images,polarization_images)
            # ---- loss function ----
...
if __name__ == '__main__':
    ...
    polarization_root = '{}/polarization/'.format(opt.train_path)
    gt_root = '{}/gt/'.format(opt.train_path)

    train_loader = get_loader(image_root, polarization_root, gt_root, batchsize=opt.batchsize, trainsize=opt.trainsize,
